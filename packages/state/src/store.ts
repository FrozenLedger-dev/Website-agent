...
export class StateStore {
  ...
  /** Durable customer-requested initial draft generations — at most one per exact (account, customer user, intake digest). See `InitialDraftRequestDocument`. */
  get initialDraftRequests(): Collection<InitialDraftRequestDocument> {
    return this.db.collection<InitialDraftRequestDocument>('initial_draft_requests');
  }
  ...
  async ensureIndexes(): Promise<void> {
    ...
    // Initial draft generation. `_id` is already the idempotency key (deterministic
    // from account + customer user + intake digest), so no separate unique index is
    // needed for that. `projectId` is unique: a project is generated by at most one
    // request, ever — this is how the generation-status route and the projects list
    // find "the" request for a project without a scan. `accountId`+`status` backs the
    // small concurrent-generation bound; `status`+`createdAt` backs work discovery,
    // the same scheduling-only shape as `semanticEditIntents`.
    await this.initialDraftRequests.createIndexes([
      { key: { projectId: 1 }, unique: true, name: 'projectId_1' },
      { key: { accountId: 1, status: 1 }, name: 'accountId_1_status_1' },
      { key: { status: 1, createdAt: 1 }, name: 'status_1_createdAt_1' },
    ]);
  }
