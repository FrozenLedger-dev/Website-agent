 ...
 *
 * The authorisation itself lives in `@statxai/policy-engine` and is tested
 * there. What is left here is what a pure decision layer must not do: read the
 * environment for an override, recognise a real runtime failure, and execute.
 */
import { afterEach, beforeEach, describe, expect, it } from 'vitest';
import { type SitePlan } from '@statxai/contracts';
import type { RoutingAuthorization } from '@statxai/policy-engine';
import { developerOverride, executeRoute, isTruncationFailure } from '../src/routing.js';
